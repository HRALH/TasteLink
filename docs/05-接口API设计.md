...
登出。服务端把当前 token 的 `jti` 写入 Redis 黑名单（TTL 取 token 剩余有效期），命中黑名单的 token 再访问受保护接口返回 401。

请求体：无（从 `Authorization: Bearer` 头取当前 token）
响应 `data`：`null`
规则：需登录（安全链未认证先 401，前端忽略即可）；Redis 缺席时静默成功，降级为仅前端清本地态；过期/非法 token 同样静默成功。前端 best-effort 调用后清本地登录态。
